...
import { TermPolicyContext } from "../contexts/termPolicyContext";
import { useParams, useNavigate } from "react-router-dom";
import Cookies from "js-cookie";
import axios from "axios";

let TermPolicyApi = () => {
    const { state, handleFunction } = useContext(TermPolicyContext)
    ...
    useEffect(() => {
        ...
        if (variableIdData !== undefined) {
            axios.get(`https://faiz-web.my.id/api/terms_policies/${variableIdData}`)
                .then((res) => {
                    let data = res.data
